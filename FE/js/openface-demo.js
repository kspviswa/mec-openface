...
function changeServerCallback() {
    ...
    function changeServerCallbackEx() {
        $(this).addClass("active").siblings().removeClass("active");
        switch ($(this).id) {
        case "local":
            socket.close();
        ...
        case "chn":
            socket.close();
            redrawPeople();
            createSocket("wss://facerec.cmusatyalab.org:9000", "Chn Innovation Lab");
            break;
        case "hyd":
            socket.close();
            redrawPeople();
            createSocket("wss://54.159.128.49:9000", "Hyd Innovation Lab");
            break;
        case "tampa":
            socket.close();
            redrawPeople();
            createSocket("wss://54.188.234.61:9000", "Tampa DC");
            break;
        default:
            alert("Unrecognized server: " + $(this).html());
        }
